...
console.log(chalk.bold.hex("#03f0fc").bold("[ Umaru ] » ") + chalk.bold.hex("#fcba03").bold("Initializing variables..."));
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        logger(`Roshi 1.2.2`, '[ Hiroshi ]');
        logger('Hey, thank you for using this Bot', '[ Hiroshi ]');
        logger("Fixed by Hiroshi", '[ Hiroshi ]');
        logger("FB: facebook.com/kimjosephbien2006", 'Fixed by Hiroshi');
        var cron = require("node-cron");
cron.schedule('0 */3 * * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("— BNS RULES/— ALWAYS USE MM/MW./— MAKE SURE YOU DONT HAVE SCAM RECORD./— NOT ALLOWED TO SEND NUDES/P*RN.CONTENT OR ANY KIND OR +18 CONTENT./— AUTO KICK IF YOU HAVE SCAM RECORD. (every 3 minute) \n\nAUTO REMIND BNSRULES", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
